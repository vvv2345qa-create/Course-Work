...
def create_attack_type_percentage_figure(results, title="Traffic classes"):
    fig = Figure(figsize=(5, 3.5), dpi=100)
    ax = fig.add_subplot(111)

    counts = Counter(row.get("attack_type", "normal") for row in results)
...
class AccessLogApp:
    ...
    def create_tree(self, parent):
        columns = ("ip", "requests", "score", "class", "true_label", "correct", "reason")
    ...
    def draw_graphs(self):
        ...
        attack_type_fig = create_attack_type_percentage_figure(self.test_results)

        self.add_figure(accuracy_fig, 0, 0)
        self.add_figure(anomaly_fig, 0, 1)
        self.add_figure(attack_type_fig, 0, 2)
